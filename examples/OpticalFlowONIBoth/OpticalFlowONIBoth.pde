// Edited for integration of ONI files for gestures failover
// 2011-10-21
...
void pixave(int x1, int y1, int x2, int y2) {
	...
	for(int i=wscreen*y+x1; i<=wscreen*y+x2; i++) {
	    if(flagimage) pix = context.depthImage().pixels[i];
	    else pix = context.rgbImage().pixels[i];
	    b=pix & 0xFF; // blue
	    pix = pix >> 8;
	    g=pix & 0xFF; // green
	    pix = pix >> 8;
...
void draw() {
    if(true){//video.available()){
	// update the cam
	context.update();

	// draw depthImageMap
	//image( context.depthImage(),0,0);
	if(flagimage) set(0,0, context.depthImage());
	else set(0,0, context.rgbImage());
	/*
	// video capture
	video.read();
	*/
